...
const SIZES = ["6Y", "8Y", "10Y", "12Y"];

const BoyswearDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchItem = async () => {
      try {
        const res = await axios.get(`https://chic-a-boo-backend.onrender.com/api/kids/boyswear/${productId}`);
        setItem(res.data);
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div>
          ...
          <div className="text-sm text-gray-700 space-y-1 mb-6">
            ...
            <p>Material: {item.material || "Unknown"}</p>
            <p>Location: {item.location ?? "Unknown"}</p>
          </div>

          <div className="mb-8">
            <p className="mb-2 font-medium text-lg">Select Size (Kids):</p>
            <div className="flex gap-3 flex-wrap">
              {SIZES.map((size) => {
                const isAvailable = item.availableSizes?.includes(size) ?? true;
                return (
                  <button
                    key={size}
