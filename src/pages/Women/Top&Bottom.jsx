...
const TopsBottoms = () => {
  const [topsbottoms, setTopsBottoms] = useState([]);
  ...
  useEffect(() => {
    const fetchTopsBottoms = async () => {
      try {
        ...
        setError('Failed to load tops and bottoms.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <p className="text-center mt-10">Loading tops & bottoms...</p>;
  if (error) return <p className="text-center mt-10 text-red-500">{error}</p>;

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <h2 className="text-2xl font-bold mb-6 text-center">Women's Tops & Bottoms</h2>

      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {topsbottoms.map((item) => {
          const inCart = isInCart(item._id);
          return (
            <div
              key={item._id}
              className="border rounded-lg p-4 shadow hover:shadow-xl transition duration-200 relative cursor-pointer"
              onClick={() => navigate(`/women/topsbottom/${item._id}`)}
            >
              <button
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  navigate(`/women/topsbottoms/${item._id}`);
                }}
