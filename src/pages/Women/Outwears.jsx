...
const Outwear = () => {
  ...
  useEffect(() => {
    const fetchOutwear = async () => {
      try {
        const res = await axios.get('https://chic-a-boo-backend.onrender.com/api/women/outwear');
        setOutwear(res.data);
      } catch (err) {
        console.error('Error fetching outwear:', err);
        setError('Failed to load outwear.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <p className="text-center mt-10">Loading outwear...</p>;
  if (error) return <p className="text-center mt-10 text-red-500">{error}</p>;

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <h2 className="text-2xl font-bold mb-6 text-center">Women's Outwear</h2>
        ...
        {outwear.map((item) => {
          ...
          return (
            <div
              ...
              onClick={() => navigate(`/women/outwear/${item._id}`)}
            >
              <button
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  navigate(`/women/outwear/${item._id}`);
                }}
