...
const EthnicWear = () => {
  ...
  useEffect(() => {
    const fetchEthnicWear = async () => {
      try {
        ...
        setError('Failed to load ethnic wear.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <p className="text-center mt-10">Loading ethnic wear...</p>;
  if (error) return <p className="text-center mt-10 text-red-500">{error}</p>;

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <h2 className="text-2xl font-bold mb-6 text-center">Women's Ethnic Wear</h2>
        ...
        {ethnicWear.map((item) => {
          ...
          return (
            <div
              ...
              onClick={() => navigate(`/women/ethnic/${item._id}`)}
            >
              <button
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  navigate(`/women/ethnic/${item._id}`);
                }}
