...
const Tshirt = () => {
  ...
  useEffect(() => {
    const fetchTshirts = async () => {
      try {
        ...
        setError('Failed to load tshirts.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <p className="text-center mt-10">Loading tshirts...</p>;
  if (error) return <p className="text-center mt-10 text-red-500">{error}</p>;

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <h2 className="text-2xl font-bold mb-6 text-center">T-Shirts</h2>
        ...
        {tshirts.map((item) => {
          ...
          return (
            <div
              ...
              onClick={() => navigate(`/tshirts/${item._id}`)}
            >
              <button
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  if (!inCart) navigate(`/tshirts/${item._id}`);
                }}
