...
const Shoe = () => {
  ...
  // Default cartItems to empty array to avoid undefined errors
  ...
  useEffect(() => {
    const fetchShoes = async () => {
      try {
        const res = await axios.get('https://chic-a-boo-backend.onrender.com/api/men/shoes');
        setShoes(res.data);
      } catch (err) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      {/* Cart Icon top right outside cards */}
      <div className="flex justify-end mb-6 items-center space-x-2">
        
      </div>

      <h2 className="text-2xl font-bold mb-6 text-center">Men's Shoes</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {shoes.map((shoe) => {
          ...
          return (
            ...
            >
              {/* Cart icon inside card top-right */}
              <button
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  if (!inCart) navigate(`/shoes/${shoe._id}`);
                }}
