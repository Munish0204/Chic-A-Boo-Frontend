...
const Shirt = () => {
  ...
  useEffect(() => {
    const fetchShirts = async () => {
      try {
        const res = await axios.get('https://chic-a-boo-backend.onrender.com/api/men/shirts');
        setShirts(res.data);
      } catch (err) {
  ...
  if (loading) return <p className="text-center mt-10">Loading shirts...</p>;
  if (error) return <p className="text-center mt-10 text-red-500">{error}</p>;

  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <h2 className="text-2xl font-bold mb-6 text-center">Shirts</h2>
        ...
        {shirts.map((item) => {
          ...
          return (
            <div
              ...
              onClick={() => navigate(`/shirts/${item._id}`)}
            >
              <button
              ...
              <button
                onClick={(e) => {
                  e.stopPropagation();
                  if (!inCart) navigate(`/shirts/${item._id}`);
                }}
