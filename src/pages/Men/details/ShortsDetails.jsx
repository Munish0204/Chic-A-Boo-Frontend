...
const ShortsDetail = () => {
    ...
    const fetchItem = async () => {
      try {
        const res = await axios.get(
          `https://chic-a-boo-backend.onrender.com/api/men/shorts/${productId}`
        );
        setItem(res.data);
      } catch (err) {
        console.error("Error fetching shorts item:", err);
  ...
  if (!item)
    return <p className="text-center mt-10 text-gray-500">Item not found.</p>;
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div>
          ...
          {/* Shorts specific details */}
          <p className="mb-1">Length: {item.length || "Knee-length"}</p>
          <p className="mb-1">Fit: {item.fit || "Regular"}</p>
          <p className="mb-1">Waistband Type: {item.waistbandType || "Elastic"}</p>
