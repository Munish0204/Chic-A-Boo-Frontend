...
// Example size conversion function (US to Indian sizes)
const usToIndian = (usSize) => {
  // Simple mapping example (adjust as needed)
  const map = {
    S: "38",
    M: "40",
    L: "42",
...
const TraditionalDetail = () => {
    ...
    const fetchItem = async () => {
      try {
        const res = await axios.get(
          `https://chic-a-boo-backend.onrender.com/api/men/traditional/${productId}`
        );
        setItem(res.data);
      } catch (err) {
        console.error("Error fetching traditional item:", err);
  ...
  if (!item)
    return (
      <p className="text-center mt-10 text-gray-500">Item not found.</p>
    );
          ...
          <div className="mb-8">
            ...
            <div className="flex gap-3 flex-wrap">
              {SIZES.map((usSize) => {
                ...
                // Check if this size is available in item.availableSizes
                const isAvailable =
                  item.availableSizes?.includes(usSize) ?? true;
                return (
                  <button
                    key={usSize}
...
export default TraditionalDetail;
