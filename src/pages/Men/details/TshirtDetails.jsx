    ...
    const fetchItem = async () => {
      try {
        const res = await axios.get(
          `https://chic-a-boo-backend.onrender.com/api/men/tshirts/${productId}`
        );
        setItem(res.data);
      } catch (err) {
        console.error("Error fetching tshirt item:", err);
    ...
    <div className="max-w-6xl mx-auto px-6 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        ...
        <div>
          ...
          {/* Tshirt specific details */}
          <p className="mb-1">
            Sleeve Length: {item.sleeveLength || "Standard"}
          </p>
          <p className="mb-1">Fit: {item.fit || "Regular"}</p>
          <p className="mb-1">Neck Type: {item.neckType || "Round"}</p>
